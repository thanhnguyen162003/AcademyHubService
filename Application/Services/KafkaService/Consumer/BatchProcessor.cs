...
namespace Application.Services.KafkaService.Consumer
{
    ...
    {
        private readonly Channel<string[]> _batchChannel;   // Channel to hold batches of messages
        private readonly int _batchSize;    // The maximum size of each batch
        private readonly TimeSpan _batchTimeout;    // The maximum wait time before triggering batch processing
        private readonly ILogger _logger;   // Logger for logging events
        private readonly List<string> _currentBatch;    // Holds the current batch of messages
        private DateTime _lastBatchTime;    // The last time a batch was processed
        ...
        public BatchProcessor(ILogger logger, int batchSize = 50, TimeSpan? batchTimeout = null)
        {
            _batchChannel = Channel.CreateUnbounded<string[]>();    // Use an unbounded channel to hold batches
            _batchSize = batchSize;
            _batchTimeout = batchTimeout ?? TimeSpan.FromSeconds(5);    // Default to 5 seconds if not provided
            _logger = logger;
